...
INPUT_SIZE = 1 * 28 * 28 
NUM_CLASSES = 10 # Still 10 classes (digits 0-9)
NUM_EPOCHS = 50
BATCH_SIZE = 1024
LEARNING_RATE = 0.001
...
transform = transforms.Compose([
    transforms.ToTensor(),
    # transforms.Normalize((0.1307,), (0.3081,)) # Common normalization for MNIST, optional for simple benchmark
])

data_root = "../data" # This will download to a 'data' folder one level up
os.makedirs(data_root, exist_ok=True)

# Use datasets.MNIST instead of datasets.CIFAR10
full_dataset = datasets.MNIST(root=data_root, download=True, train=True, transform=transform)
...
writer_filename = (
    f"./tbsummary/mnist/standalone_" # Changed from cifar10 to mnist
...
# Add graph (needs a dummy input of the correct shape for MNIST)
dummy_input_for_graph = torch.randn(1, 1, 28, 28).to(device) # Changed shape to 1x28x28 for MNIST
...
with torch.profiler.profile(
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for batch_idx, (inputs, labels) in enumerate(train_loader):
            ...
            # Note: With NUM_EPOCHS = 10, this will not be triggered unless you increase NUM_EPOCHS.
            # I've kept the original logic for consistency.
            if (epoch + 1) % 25 == 0: 
        ...
        # Only log GPU memory if CUDA is actually being used
        if device.type == 'cuda':
...
hparam_dict = {
    ...
    "dataset": "MNIST", # Added dataset info
}
metric_dict = {
