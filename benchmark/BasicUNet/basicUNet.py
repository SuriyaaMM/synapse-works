    ...
    for batch_idx, (features, labels) in enumerate(train_loader):
        ...
        for name, param in model.named_parameters():
            if param.grad is not None:
                writer.add_histogram(f"grads/{name.replace('.', '/')}",
                                     param.grad, current_global_step)
        ...
        if (
                current_epoch + 1
        ) % log_frequency_epoch == 0 and batch_idx == 0:  # Log once per relevant epoch start
            total_gradient_norm = 0.0
            for p in model.parameters():
                if p.grad is not None:
                    total_gradient_norm += p.grad.data.norm(2).item()**2
            total_gradient_norm = total_gradient_norm**0.5
            writer.add_scalar("Gradient Norm", total_gradient_norm,
                              current_global_step)
...
def run_training(model,
    ...
    with profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA],
                 ...
                 on_trace_ready=torch.profiler.tensorboard_trace_handler(
                     writer_filename)) as prof:
        with record_function(
                "model_training"):  # Label for the profiling section
                ...
                if (epoch + 1) % LOG_EPOCH_HIST_AND_VAL_FREQ == 0:
                    for name, param in model.named_parameters():
                        writer.add_histogram(
                            f"weights/{name.replace('.', '/')}", param, epoch)

                    # Perform validation
                    val_loss, val_correct, val_total = validate_epoch(
