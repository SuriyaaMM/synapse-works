...
class MiniUNet(nn.Module):
    def __init__(self):
        ...
        self.dec1_conv1 = nn.Conv2d(256, 128, 3, padding=1) # Input channels are 256 due to concatenation (128 from upsample + 128 from skip)
        self.dec1_conv2 = nn.Conv2d(128, 128, 3, padding=1)

        # Decoder 2
        self.up2 = nn.ConvTranspose2d(128, 64, 2, stride=2)
        self.dec2_conv1 = nn.Conv2d(128, 64, 3, padding=1) # Input channels are 128 due to concatenation (64 from upsample + 64 from skip)
    ...
    def forward(self, x):
        ...
        d1_cat = torch.cat([up_b, e2], dim=1) # Concatenate along channel dimension
        ...
        d2_cat = torch.cat([up_d1, e1], dim=1) # Concatenate along channel dimension
        d2_c1 = F.relu(self.dec2_conv1(d2_cat))
        d2 = F.relu(self.dec2_conv2(d2_c1))

        # Final output for segmentation-like task (1 channel)
        out_segmentation = self.final_conv(d2) # No ReLU here as it's typically before the final output layer for classification/regression

        # Flatten and Linear for MNIST classification
        flat = self.flatten(out_segmentation)
        out_classification = self.linear(flat) # No activation here, as CrossEntropyLoss expects logits
...
def main():
    ...
    optimizer = optim.SGD(net.parameters(), lr=0.0003, momentum=0.9, weight_decay=0.0001, nesterov=True) # Added momentum, weight_decay, nesterov
    ...
    loader = DataLoader(train_dataset, batch_size=1024, shuffle=True, num_workers=4, pin_memory=True)
    ...
    for epoch in range(10):
        ...
        for i, (x, y) in enumerate(loader):
            ...
            writer.add_scalar("Batch Time", batch_time, epoch * len(loader) + i)
        ...
        acc = correct / total
        print(f"Epoch {epoch + 1}: Loss = {total_loss:.4f}, Accuracy = {acc*100:.2f}%, Time = {epoch_time:.4f}s")
