...
class ConvMNIST(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.net = nn.Sequential(nn.Conv2d(1, 32, 3, padding=1), nn.ReLU(),
                                 nn.MaxPool2d(2),
...
# ----------------- Train Loop ------------------
def train_one_epoch(model, loader, optimizer, loss_fn, writer, device, epoch,
                    config):
    ...
    for step, (x, y) in enumerate(loader):
        ...
        if config['log_grad_norm'] and (epoch + 1) % config['log_period'] == 0:
            total_norm = sum((p.grad.norm(2)**2 for p in model.parameters()
    ...
    logging.info(
        f"[Epoch {epoch+1}] Train Loss: {avg_loss:.4f}, Accuracy: {acc:.4f}")
...
def validate(model, loader, loss_fn, writer, device, epoch):
    ...
    logging.info(
        f"[Epoch {epoch+1}] Val Loss: {avg_loss:.4f}, Accuracy: {acc:.4f}")
