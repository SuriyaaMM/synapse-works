...
plt.rcParams.update({
    ...
    'text.usetex': False, # Set to True if LaTeX is installed and desired for text rendering
    'axes.linewidth': 1.2,
    'grid.linewidth': 0.8,
    'lines.linewidth': 2,
    'patch.linewidth': 1.2,
...
try:
    ...
    print("benchmarkData.csv not found. Creating dummy data for demonstration.")
    data = {
...
df['development_effort'] = df.apply(
    lambda row: row['clicks'] if row['whom'] == 'synapse-works' else row['loc'],
    axis=1
)
df['development_effort_label'] = df.apply(
    lambda row: 'Clicks' if row['whom'] == 'synapse-works' else 'Lines of Code',
    axis=1
)
...
bars1 = ax1.bar(x - width/2, df['average_cpu_time_s'], width,
                label='CPU Time', color=colors['synapse-works'], alpha=0.8,
                edgecolor='black', linewidth=1)
ax1.set_ylabel('CPU Time (seconds)', fontweight='bold', fontsize=12, color=colors['synapse-works'])
...
ax1_twin = ax1.twinx()
bars2 = ax1_twin.bar(x + width/2, df['average_cuda_time_s'], width,
                     label='CUDA Time', color=colors['standalone'], alpha=0.8,
                     edgecolor='black', linewidth=1)
ax1_twin.set_ylabel('CUDA Time (seconds)', fontweight='bold', fontsize=12, color=colors['standalone'])
...
for bar in bars1:
    ...
    ax1.annotate(f'{height:.2f}s',
                 ...
                 ha='center', va='bottom', fontsize=9, fontweight='bold', color=colors['synapse-works'])
...
for bar in bars2:
    ...
    ax1_twin.annotate(f'{height:.2f}s',
                      xy=(bar.get_x() + bar.get_width() / 2, height),
                      xytext=(0, 3), textcoords="offset points",
                      ha='center', va='bottom', fontsize=9, fontweight='bold', color=colors['standalone'])
...
ax1_twin.legend(lines + lines2, labels + labels2, loc='upper right', frameon=True, fancybox=True, shadow=True)

plt.tight_layout()
plt.savefig('1_performance_metrics.png', dpi=300, bbox_inches='tight',
            facecolor='white', edgecolor='none')
plt.savefig('1_performance_metrics.pdf', bbox_inches='tight',
            facecolor='white', edgecolor='none')
plt.show()
...
# ============================================================================
fig2, (ax2a, ax2b) = plt.subplots(1, 2, figsize=(12, 5), facecolor='white') # Changed to 2 subplots
fig2.suptitle('Development & Construction Effort Analysis', fontweight='bold', fontsize=16, y=1.02)

# Construction Time (remains the same)
bars_const = ax2a.bar(df['model_config'], df['average_construction_time_s'],
                      color=colors['conv'], alpha=0.8, edgecolor='black', linewidth=1)
...
bars_effort = ax2b.bar(df['model_config'], df['development_effort'],
                        color='#9467bd', alpha=0.8, edgecolor='black', linewidth=1) # Using a new color
...
for ax, bars, metric_col in [(ax2a, bars_const, None), (ax2b, bars_effort, 'development_effort_label')]:
...
plt.tight_layout()
plt.savefig('2_construction_effort.png', dpi=300, bbox_inches='tight',
            facecolor='white', edgecolor='none')
plt.savefig('2_construction_effort.pdf', bbox_inches='tight',
            facecolor='white', edgecolor='none')
plt.show()
...
fig3.suptitle('Model Quality & Performance Trade-offs', fontweight='bold', fontsize=16, y=1.02)

# Accuracy Comparison
bars_acc = ax3a.bar(df['model_config'], df['average_accuracy_percent'],
                    color='#8c564b', alpha=0.8, edgecolor='black', linewidth=0.15)
ax3a.set_title('Model Accuracy Comparison', fontweight='bold', pad=15)
ax3a.set_ylabel('Accuracy (%)', fontweight='bold')
ax3a.set_ylim(93, 100)
...
legend_elements = [
    Line2D([0], [0], marker='o', color='w', markerfacecolor=colors['synapse-works'],
           markersize=10, label='Synapse-works', markeredgecolor='black', markeredgewidth=1.5),
    Line2D([0], [0], marker='s', color='w', markerfacecolor=colors['standalone'],
           markersize=10, label='Standalone', markeredgecolor='black', markeredgewidth=1.5),
    Line2D([0], [0], marker='o', color='w', markerfacecolor='gray',
           markersize=8, label='Conv Model', markeredgecolor='black'),
    Line2D([0], [0], marker='s', color='w', markerfacecolor='gray',
           markersize=8, label='Linear Model', markeredgecolor='black')
]
ax3b.legend(handles=legend_elements, loc='lower right', frameon=True,
           fancybox=True, shadow=True, fontsize=10)

plt.tight_layout()
plt.savefig('3_accuracy_quality.png', dpi=300, bbox_inches='tight',
            facecolor='white', edgecolor='none')
plt.savefig('3_accuracy_quality.pdf', bbox_inches='tight',
            facecolor='white', edgecolor='none')
plt.show()
